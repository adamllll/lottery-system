...
- **抽奖模块**：基于 RabbitMQ 消息队列实现异步抽奖，支持后端校验抽奖请求合法性（活动状态、奖品状态、中奖人数）、中奖记录持久化、Redis 缓存加速查询、邮件异步通知中奖者。采用死信队列（DLX）实现消息重试机制，保障抽奖流程的可靠性。
...
| 用例编号 | 测试场景 | 前置条件 | 测试步骤 | 预期结果 | 优先级 |
|---------|---------|---------|---------|---------|-------|
| D-001 | 发起抽奖-后端校验通过 | 活动 RUNNING，奖品 INIT | 提交抽奖参数（活动ID、奖品ID、中奖者列表） | 服务端校验活动状态、奖品状态与中奖人数通过后，消息发送至 MQ，接口返回成功 | P0 |
...
| D-010 | 后端校验-活动已结束 | 活动状态为 END | 提交抽奖参数 | 服务端拒绝处理该抽奖请求 | P0 |
| D-011 | 后端校验-奖品已抽完 | 奖品状态为 END | 提交抽奖参数 | 服务端拒绝处理该抽奖请求 | P0 |
| D-012 | 后端校验-中奖人数不匹配 | 奖项数量与中奖列表数量不一致 | 提交抽奖参数 | 服务端拒绝处理该抽奖请求 | P0 |
...
本测试类是系统最核心的集成测试，共包含 18 个测试方法，覆盖抽奖的正向流程、后端中奖校验、异常回滚、死信重试等关键场景。

当前抽奖结果的最终合法性校验已下沉到消费者侧 `checkDrawPrizeStatus()`，用于拦截活动已结束、奖品已抽完、中奖人数与奖项数量不一致等非法或重复请求；本报告中的抽奖链路与接口回归口径已统一按后端校验逻辑描述。
...
#### 4.2.8 testCheckDrawPrizeStatusShouldFailWhenActivityEnded — 活动结束拦截

直接调用 `checkDrawPrizeStatus()`，验证活动状态已为 `END` 时，后端会拒绝继续处理该抽奖请求。

#### 4.2.9 testCheckDrawPrizeStatusShouldFailWhenPrizeEnded — 奖品抽完拦截

直接调用 `checkDrawPrizeStatus()`，验证奖品状态已为 `END` 时，后端会拒绝继续处理该抽奖请求。

#### 4.2.10 testCheckDrawPrizeStatusShouldFailWhenWinnerCountMismatch — 中奖人数不匹配拦截

直接调用 `checkDrawPrizeStatus()`，验证中奖名单数量与活动奖项配置数量不一致时，后端会拒绝继续处理该抽奖请求。

#### 4.2.11 testShowWinningRecords — 中奖记录查询
...
| 测试类 | 测试内容 | 测试方法数 |
|-------|---------|----------|
| `ActivityDetailSerializationTest` | 抽奖页活动详情序列化字段兼容性 | 2 |
| `ActivityServiceImplCacheTest` | 活动详情缓存命中读取 | 1 |
| `AppConfigTest` | 静态图片资源放行规则 | 1 |
| `DirectRabbitConfigTest` | RabbitMQ 交换机名称配置 | 2 |
...
| `UserTest` | 用户列表查询联调验证 | 1 |
| `LotterySystemApplicationTests` | Spring 上下文启动验证 | 1 |
...
| 测试类别 | 测试类数 | 测试方法数 | 备注 |
|---------|---------|----------|------|
| 抽奖核心流程 | 1 | 11 | 覆盖正向处理、后端中奖校验、状态流转、异常回滚、DLX 重试、中奖记录查询 |
| 短信验证码 | 1 | 5 | 自动用例 2 个，手工辅助用例 3 个 |
| 抽奖页兼容与缓存 | 3 | 4 | 覆盖序列化字段、缓存命中、静态图片资源访问 |
| RabbitMQ 配置 | 1 | 2 | 覆盖业务交换机与死信交换机名称 |
| 其他基础设施与工具 | 10 | 14 | 覆盖 Redis、JWT、加密、SQL、JSON、邮件、短信、日志、上下文启动 |
| **合计** | **16** | **36** | **按当前 `src/test/java` 测试类与 `@Test` 方法统计** |

说明：本节统计按当前仓库测试源码整理，已将近期提交新增的抽奖页兼容性、缓存与 RabbitMQ 配置回归用例纳入清单。
...
| 接口 | 方法 | 路径 | 测试要点 |
|------|------|------|---------|
| 发起抽奖 | POST | `/draw-prize` | 正常抽奖、活动已结束、奖品已抽完、中奖人数与奖项数量不一致、重复请求兜底 |
...
```mermaid
pie title 自动化测试方法分布（共 36 个）
    "抽奖核心流程 (DrawPrizeTest)" : 11
...
基于抽奖系统用户管理、活动管理、奖品管理、抽奖执行 4 大核心模块，共设计 **39 条接口测试用例** + **39 条功能测试用例**，累计 78 条；本报告记录了 76 条有效执行结果，遗留待优化问题 2 条（详见 Bug 描述章节）。

### 自动化测试

针对抽奖核心流程、短信验证码服务、抽奖页兼容性回归及基础设施组件，当前仓库共包含 **16 个测试类、36 个自动化测试方法**：

- **抽奖核心流程**（DrawPrizeTest）：18 个测试方法，覆盖正向流程、后端中奖校验、状态流转、异常回滚（两种异常类型）、死信队列重试、中奖记录查询等关键场景
- **短信验证码**（VerificationCodeServiceITest）：5 个测试方法，覆盖正例校验、错误码、过期码、重发失效、频控限制等场景，其中 3 个为手工辅助用例
- **抽奖页兼容与缓存回归**：新增 4 个测试方法，覆盖活动详情返回字段、缓存命中读取、静态图片资源放行、RabbitMQ 交换机配置等近期提交引入的兼容性点
- **其他基础设施**：其余 14 个测试方法覆盖 Redis、JWT、AES/SHA256 加密、MyBatis 查询、JSON 序列化、邮件发送、短信 API、日志配置、上下文启动等能力
